...
UPLOAD_DIR = os.path.join(os.path.dirname(__file__), "../../../uploads/modalites_application")
os.makedirs(UPLOAD_DIR, exist_ok=True)

LOAD_OPTS = [
    selectinload(ModalitesChapitre.sections),
    selectinload(ModalitesChapitre.articles).selectinload(ModalitesArticle.section),
]
...
def num_chapitre(n: int) -> str: return "premier" if n == 1 else to_roman(n)
def num_section(n: int)  -> str: return "première" if n == 1 else to_roman(n)
def num_article(n: int)  -> str: return "premier" if n == 1 else str(n)


def article_to_dict(a: ModalitesArticle) -> dict:
    return {
        "id":          str(a.id),
        "numero":      a.numero,
...
def section_to_dict(s: ModalitesSection, articles: list) -> dict:
    return {
        "id":          str(s.id),
        "numero":      s.numero,
...
def chapitre_to_dict(c: ModalitesChapitre) -> dict:
...
async def upload_pdf(
    titre:   str        = Form("Modalités d'application du code des investissements"),
    version: str        = Form(""),
    ...
    if old:
        ...
        await db.delete(old)
    pdf = ModalitesPdf(titre=titre, version=version or None, fichier_nom=fichier.filename, fichier_path=dest)
    db.add(pdf)
    await db.flush()
...
async def renommer_pdf(pdf_id: UUID, payload: dict, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_admin)):
    res = await db.execute(select(ModalitesPdf).where(ModalitesPdf.id == pdf_id))
    p   = res.scalar_one_or_none()
...
# ── Lecture complète ──────────────────────────────────────────────────────────
@router.get("")
async def get_modalites(db: AsyncSession = Depends(get_db)):
    res = await db.execute(
        select(ModalitesChapitre).options(*LOAD_OPTS).order_by(ModalitesChapitre.numero)
    )
    return [chapitre_to_dict(c) for c in res.scalars().all()]


# ── Recherche full-text ───────────────────────────────────────────────────────
@router.get("/search")
async def search_modalites(q: str, db: AsyncSession = Depends(get_db)):
    if len(q) < 2: return []
...
async def creer_chapitre(payload: dict, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_admin)):
    c = ModalitesChapitre(numero=payload["numero"], titre=payload["titre"])
    db.add(c); await db.flush()
    res = await db.execute(select(ModalitesChapitre).options(*LOAD_OPTS).where(ModalitesChapitre.id == c.id))
...
async def modifier_chapitre(chap_id: UUID, payload: dict, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_admin)):
    res = await db.execute(select(ModalitesChapitre).where(ModalitesChapitre.id == chap_id))
    ...
    await db.flush()
    res = await db.execute(select(ModalitesChapitre).options(*LOAD_OPTS).where(ModalitesChapitre.id == chap_id))
    return chapitre_to_dict(res.scalar_one())
...
async def creer_section(chap_id: UUID, payload: dict, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_admin)):
    s = ModalitesSection(chapitre_id=chap_id, numero=payload["numero"], titre=payload["titre"])
...
async def modifier_section(sec_id: UUID, payload: dict, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_admin)):
    res = await db.execute(select(ModalitesSection).where(ModalitesSection.id == sec_id))
...
async def modifier_article(art_id: UUID, payload: dict, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_admin)):
    res = await db.execute(select(ModalitesArticle).where(ModalitesArticle.id == art_id))
