...
async def importer_bmce(
    ...
    # L'identifiant reste donc stable, les flux déjà rattachés le restent, et
    # aucune contrainte (unicité de periode ni clé étrangère) n'est violée.
    res = await db.execute(text(
        "INSERT INTO bmce_bulletins (periode, fichier_nom, mois_couverts, rapport) "
        "VALUES (:p, :f, :m, :r) "
        "ON CONFLICT (periode) DO UPDATE SET "
        "  fichier_nom = EXCLUDED.fichier_nom, mois_couverts = EXCLUDED.mois_couverts, "
        "  rapport = EXCLUDED.rapport, importe_le = now() "
        "RETURNING id"),
