    ...
    <div ref={ref} style={{ position: "relative", ...style }}>
      <div onClick={() => { if (disabled) return; setOpen(o => !o); }}
        style={{ ...inputBase, minHeight: 40, display: "flex", alignItems: "flex-start",
          ...
          background: disabled ? "var(--champ)" : inputBase.background,
          border: `1px solid ${open ? "var(--bleu)" : "var(--bordure-forte)"}`,
          transition: "border-color 0.2s", flexWrap: "wrap", gap: 4, paddingRight: 32 }}>
        {selected.length === 0 ? (
          <span style={{ color: "var(--gris)", lineHeight: "22px" }}>{disabled ? (disabledHint || placeholder) : placeholder}</span>
        ) : (
      ...
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, zIndex: 200,
          ...
          boxShadow: "var(--ombre-2)", maxHeight: 320, overflowY: "auto" }}>
              ...
              {list.map(p => {
                ...
                return (
                  <div key={p.id}
                    ...
                    onMouseLeave={e => { e.currentTarget.style.background = isSelected ? "rgb(var(--bleu-rgb) / 0.06)" : "transparent"; }}>
                    <div style={{ width: 16, height: 16, borderRadius: 4, flexShrink: 0,
                      border: `2px solid ${isSelected ? "var(--bleu)" : "var(--bordure-forte)"}`,
                      background: isSelected ? "var(--bleu-action)" : "transparent",
                    ...
                    <span style={{ color: isSelected ? "var(--bleu)" : "var(--encre)", fontWeight: isSelected ? 600 : 400 }}>{p.nom_fr}</span>
                  </div>
                );
              })}
            </div>
