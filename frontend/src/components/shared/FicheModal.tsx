...
export default function FicheModal({ titre, badges, onClose, zIndex = 400, maxWidth = 640, actions, children }: {
  ...
  return (
    <div
      ...
      style={{ position: "fixed", inset: 0, background: "rgba(2,20,38,0.45)", backdropFilter: "blur(8px)", zIndex, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
...
[data-rte] ul{padding-left:20px;list-style-type:disc}[data-rte] ol{padding-left:20px;list-style-type:decimal}[data-rte] li{margin-bottom:2px}`}</style>
      <div onClick={e => e.stopPropagation()}
        style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth, maxHeight: "92vh", display: "flex", flexDirection: "column", overflow: "hidden", boxShadow: "var(--ombre-2)", animation: "vueIn 0.22s ease" }}>
        {/* Liseré d'accent */}
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, padding: "18px 28px 16px", borderBottom: "1px solid #F2F0EF", flexShrink: 0 }}>
          <div style={{ minWidth: 0, flex: 1 }}>
            <h2 style={{ fontWeight: 800, fontSize: "1.1rem", color: "#1a1a2e", lineHeight: 1.3, margin: 0 }}>{titre}</h2>
...
export function FicheSection({ titre, count, children }: { titre: React.ReactNode; count?: number; children: React.ReactNode }) {
  return (
    <section>
      <p style={{ fontSize: 10.5, fontWeight: 700, color: "#004f91", letterSpacing: "0.14em", textTransform: "uppercase", marginBottom: 10 }}>
        {titre}{typeof count === "number" ? <span style={{ color: "#C5BFBB", fontWeight: 700, marginLeft: 7 }}>{count}</span> : null}
      </p>
      {children}
    </section>
  );
...
export function FicheBloc({ label, children, full }: { label: string; children: React.ReactNode; full?: boolean }) {
  return (
    <div style={{ ...fond_bleu, borderRadius: 12, padding: "9px 12px", minWidth: 0, gridColumn: full ? "1/-1" : undefined }}>
      <p style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.1em", color: "#004f91", textTransform: "uppercase", marginBottom: 3 }}>{label}</p>
      {children}
    </div>
  );
}
export function FicheValeur({ children, vide, fort }: { children?: React.ReactNode; vide?: boolean; fort?: boolean }) {
  return <p style={{ fontSize: fort ? 13 : 12.5, fontWeight: fort ? 700 : 600, color: vide ? "#9aa5b4" : "#1a1a2e" }}>{children}</p>;
}
export function FicheLien({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <a href={href.startsWith("http") ? href : `https://${href}`} target="_blank" rel="noopener noreferrer"
      style={{ fontSize: 12.5, fontWeight: 600, color: "#004f91", textDecoration: "none", wordBreak: "break-all" }}>
      {children}
    </a>
  );
}
...
export function FicheCarteNeutre({ children, style }: { children: React.ReactNode; style?: React.CSSProperties }) {
  return <div style={{ background: "#FAFAF9", border: "1px solid #F0EEEC", borderRadius: 12, padding: "12px 14px", ...style }}>{children}</div>;
}
...
export function FicheTexteRiche({ html }: { html: string }) {
  return (
    <FicheCarteNeutre style={{ padding: "13px 15px" }}>
      <div data-rte dangerouslySetInnerHTML={{ __html: html }} style={{ fontSize: 13, color: "#4a5568", lineHeight: 1.7 }} />
    </FicheCarteNeutre>
  );
}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
        {fichiers.map((f: any) => (
          <a key={f.id} href={hrefDe(f)} target="_blank" rel="noopener noreferrer"
            ...
            onMouseEnter={ev => (ev.currentTarget.style.borderColor = "rgba(0,79,145,0.35)")}
            onMouseLeave={ev => (ev.currentTarget.style.borderColor = "rgba(0,79,145,0.16)")}>
            <FileText size={13} style={{ color: "#004f91", flexShrink: 0 }} />
            <span style={{ fontSize: 12.5, color: "#004f91", fontWeight: 600 }}>{f.titre || f.fichier_nom || f.nom || "Document"}</span>
          </a>
        ))}
      </div>
    </FicheSection>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {data.map(sec => (
        <div key={sec.cle}>
          ...
          {!!sec.enfants?.length && (
            <div style={{ paddingLeft: 20, borderLeft: "2px solid rgba(0,79,145,0.15)", display: "flex", flexDirection: "column", gap: 5 }}>
              {sec.enfants.map(bra => (
                <div key={bra.cle}>
