...
export function Skeleton({ w, h, r = 8, style }: {
  ...
  return (
    <>
      ...
      <div style={{ width: w ?? "100%", height: h ?? 14, borderRadius: r, background: "var(--fond-creux2)", animation: "skPulse 1.4s ease-in-out infinite", ...style }} />
    </>
  );
}
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: `repeat(${n},1fr)`, gap: 12 }}>
      {Array.from({ length: n }).map((_, i) => (
        <div key={i} style={{ background: "var(--carte)", border: "1px solid var(--bordure)", borderRadius: 14, padding: "14px 16px", height, boxSizing: "border-box", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
...
export function SkeletonChart({ height = 300 }: { height?: number }) {
  return (
    <div style={{ background: "var(--carte)", border: "1px solid var(--bordure)", borderRadius: 14, padding: "16px 18px", boxSizing: "border-box" }}>
