...
function Badge({ value }: { value: string }) {
  ...
  if (!style) return <span style={{ fontSize: "var(--t-12)" }}>{value}</span>;
  return (
    <span style={{ ...style, padding: "2px 9px", borderRadius: 999, fontSize: "var(--t-11)", fontWeight: 600, whiteSpace: "nowrap" }}>
      {value}
    </span>
  );
}
    ...
    <div style={{
      ...
      <div style={{ padding: "16px 20px 14px", background: "var(--carte)", borderBottom: "1px solid var(--bordure)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
          <h2 style={{ fontSize: "var(--t-14)", fontWeight: 800, color: "var(--encre)", margin: 0, minWidth: 0 }}>{titre}</h2>
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexShrink: 0 }}>
            ...
            <button onClick={exportXLSX} title={`Exporter ${sorted.length} lignes en Excel`}
              style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px",
                ...
                cursor: "pointer", fontSize: "var(--t-115)", color: "var(--sur-bleu)", fontWeight: 700, fontFamily: "var(--font-google-sans)" }}>
      ...
      <div style={{ overflowX: "auto" }}>
        {loading ? (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 50, gap: 10, color: "var(--gris)" }}>
            ...
            <span style={{ fontSize: "var(--t-13)" }}>Chargement…</span>
          </div>
        ) : error ? (
          <div style={{ padding: 40, textAlign: "center" as const }}>
            <p style={{ fontSize: "var(--t-13)", color: "var(--danger)" }}>Erreur lors du chargement.</p>
          </div>
        ) : sorted.length === 0 ? (
          <div style={{ padding: 50, textAlign: "center" as const }}>
            <p style={{ fontSize: "var(--t-13)", color: "var(--gris)" }}>
              {search ? `Aucun résultat pour "${search}"` : "Aucune donnée disponible."}
            </p>
          </div>
        ) : (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "var(--t-12)", minWidth: 600 }}>
            <thead>
                  ...
                  return (
                    <th key={col} onClick={() => toggleSort(col)}
                      style={{
                        ...
                        fontSize: "var(--t-10)", fontWeight: 800, color: active ? "var(--bleu)" : "var(--texte)",
        ...
        <div style={{ padding: "10px 20px", borderTop: "1px solid var(--bordure)",
          ...
          {rowsLimit !== "Tout" && sorted.length > (rowsLimit as number) && (
            <button onClick={() => setRowsLimit("Tout")}
              style={{ fontSize: "var(--t-11)", fontWeight: 700, color: "var(--bleu)", background: "rgb(var(--bleu-rgb) / 0.06)",
          ...
          {rowsLimit === "Tout" && (
            <button onClick={() => setRowsLimit(7)}
              style={{ fontSize: "var(--t-11)", fontWeight: 600, color: "var(--texte)", background: "var(--carte)",
