...
export function GrapheModal({ open, onClose, titre, sous_titre, unite, source, children, analyse, series, grapheId }: any) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgb(var(--encre-rgb) / 0.45)", backdropFilter:"blur(8px)", zIndex:500, display:"flex", alignItems:"center", justifyContent:"center", padding:32 }}>
      ...
      <div {...dial} aria-label={titre} onClick={e=>e.stopPropagation()} style={{ background:"var(--carte)", borderRadius:20, width:"100%", maxWidth:1100, maxHeight:"92vh", display:"flex", flexDirection:"column" as const, overflow:"hidden", boxShadow:"var(--ombre-2)", animation:"vueIn 0.22s ease" }}>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:16 }}>
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" as const, minWidth:0 }}>
                ...
                {anneesRange && (
                  <span style={{ flexShrink:0, fontSize: "var(--t-11)", fontWeight:700, color:"var(--texte)", background:"rgb(var(--gris-rgb) / 0.16)", padding:"3px 10px", borderRadius:999, whiteSpace:"nowrap" as const }}>
                    {anneesRange}
                  </span>
                )}
                {series?.length > 0 && (
        ...
        <div style={{ padding:"22px 28px", overflowY:"auto" as const, flex:1 }}>
          ...
          {analyse && (
            <div style={{ marginTop:22 }}>
              <p style={{ fontSize: "var(--t-105)", fontWeight:700, color:"var(--bleu)", letterSpacing:"0.14em", textTransform:"uppercase" as const, marginBottom:10 }}>{analyse.titre}</p>
              <div style={{ background:"var(--carte-douce)", border:"1px solid var(--bordure)", borderRadius:12, padding:"14px 18px" }}>
                <p style={{ fontSize: "var(--t-13)", color:"var(--texte)", lineHeight:1.7 }}>{analyse.commentaire}</p>
              </div>
            </div>
          )}
        </div>
