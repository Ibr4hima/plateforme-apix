...
// ── Modal graphe plein écran (page IDE) ───────────────────────────────────────
export function GrapheModal({ open, onClose, titre, sous_titre, children, analyse, series, grapheId }: any) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgb(var(--encre-rgb) / 0.45)", backdropFilter:"blur(8px)", zIndex:500, display:"flex", alignItems:"center", justifyContent:"center", padding:32 }}>
      ...
      <div role="dialog" aria-modal="true" aria-label={titre} onClick={e=>e.stopPropagation()} style={{ background:"var(--carte)", borderRadius:20, width:"100%", maxWidth:1100, maxHeight:"92vh", display:"flex", flexDirection:"column" as const, overflow:"hidden", boxShadow:"var(--ombre-2)", animation:"vueIn 0.22s ease" }}>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:16 }}>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ display:"flex", alignItems:"center", gap:10, minWidth:0 }}>
                <h2 style={{ fontWeight:800, fontSize:"1.1rem", color:"var(--encre)", margin:0, lineHeight:1.35, minWidth:0 }}>{titre}</h2>
                {anneesRange && (
                  <span style={{ flexShrink:0, fontSize:11, fontWeight:700, color:"var(--texte)", background:"var(--fond-creux2)", padding:"3px 10px", borderRadius:999, whiteSpace:"nowrap" as const }}>
                    {anneesRange}
                  </span>
                )}
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" as const, marginTop:8 }}>
                ...
                {sous_titre && <span style={{ fontSize:11.5, color:"var(--gris)", fontWeight:500 }}>{sous_titre}</span>}
              </div>
            </div>
        ...
        <div style={{ padding:"22px 28px", overflowY:"auto" as const, flex:1 }}>
          ...
          {analyse && (
            <div style={{ marginTop:22 }}>
              <p style={{ fontSize:10.5, fontWeight:700, color:"var(--bleu)", letterSpacing:"0.14em", textTransform:"uppercase" as const, marginBottom:10 }}>{analyse.titre}</p>
              <div style={{ background:"var(--carte-douce)", border:"1px solid var(--bordure)", borderRadius:12, padding:"14px 18px" }}>
                <p style={{ fontSize:13, color:"var(--texte)", lineHeight:1.7 }}>{analyse.commentaire}</p>
              </div>
            </div>
          )}
        </div>
        ...
        {/* Pied fixe */}
        <div style={{ padding:"14px 28px", borderTop:"1px solid var(--bordure)", background:"var(--carte-douce)", display:"flex", justifyContent:"flex-end", gap:10, flexShrink:0 }}>
