...
const Ico = ({ nom, taille = 19, couleur = "#6b7684", rempli = 0 }: {
  ...
  return (
    <aside className="nav-admin" aria-label="Navigation de l'administration"
      style={{ width: L, flexShrink: 0, height: "100vh", position: "sticky", top: 0, zIndex: 40,
        background: "#fff", borderRight: "1px solid #E8E5E3", display: "flex", flexDirection: "column",
      ...
      <style>{`
        ...
        .nav-admin ::-webkit-scrollbar-thumb { background: #E8E5E3; border-radius: 99px; }
        .nav-admin ::-webkit-scrollbar-thumb:hover { background: #C5BFBB; }
        .nav-lien { position: relative; display: flex; align-items: center; gap: 11px; padding: 8px 10px;
          ...
          cursor: pointer; text-align: left; font-family: inherit; color: #4a5568;
          transition: background 0.13s, color 0.13s; }
        .nav-lien:hover { background: #F7F6F5; color: #101a2e; }
        .nav-lien:focus-visible { outline: 2px solid rgba(202,99,31,0.55); outline-offset: 1px; }
        .nav-lien.actif { background: rgba(202,99,31,0.09); color: ${ORANGE}; }
        .nav-lien.bloque { opacity: 0.4; cursor: not-allowed; }
        .nav-lien.bloque:hover { background: transparent; color: #4a5568; }
        .nav-lien.rouge:hover { background: rgba(220,38,38,0.06); }
        .nav-txt { font-size: 12.5px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
      `}</style>

      {/* ── En-tête : identité de l'espace ── */}
      <div style={{ padding: replie ? "16px 8px 12px" : "18px 16px 14px", borderBottom: "1px solid #F2F0EF",
        ...
        {replie ? (
          <span title="Espace d'administration" style={{ width: 32, height: 32, borderRadius: 9, flexShrink: 0,
            background: `linear-gradient(135deg,${ORANGE},#e28f46)`, color: "#fff", display: "flex",
          ...
          <>
            ...
            <span style={{ fontSize: 8.5, fontWeight: 800, color: ORANGE, letterSpacing: "0.16em",
              textTransform: "uppercase", background: "rgba(202,99,31,0.10)", padding: "3px 7px",
      ...
      <nav style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: replie ? "10px 10px 14px" : "10px 12px 14px" }}>
        {MODULES_ADMIN.map((item, i) => {
          if (item.type === "section") {
            ...
            if (replie) return <div key={`s${i}`} style={{ height: 1, background: "#F2F0EF", margin: i === 0 ? "2px 6px 8px" : "12px 6px 8px" }} />;
            return (
              <div key={`s${i}`} style={{ display: "flex", alignItems: "center", gap: 8, padding: i === 0 ? "2px 10px 7px" : "18px 10px 7px" }}>
                <span style={{ fontSize: 9, fontWeight: 800, color: "#9aa5b4", letterSpacing: "0.15em",
          ...
          if (bloque) {
            return (
              <div key={item.href} className="nav-lien bloque"
                ...
                <Ico nom={item.icon} couleur="#9aa5b4" />
                {!replie && <span className="nav-txt" style={{ flex: 1, fontWeight: 500 }}>{item.label}</span>}
                {!replie && <Lock size={12} style={{ color: "#C5BFBB", flexShrink: 0 }} />}
              </div>
            );
          }
          ...
          return (
            ...
            <Link key={item.href} href={item.href} className={`nav-lien${actif ? " actif" : ""}`}
              ...
              {actif && <span aria-hidden style={{ position: "absolute", left: replie ? 2 : -4, top: "50%",
                transform: "translateY(-50%)", width: 3, height: 18, borderRadius: 999, background: ORANGE }} />}
              <Ico nom={item.icon} couleur={actif ? ORANGE : "#6b7684"} rempli={actif ? 1 : 0} />
      ...
      <div style={{ borderTop: "1px solid #F2F0EF", padding: replie ? "8px 10px 10px" : "8px 12px 10px", flexShrink: 0 }}>
        {session?.user && (
          <div title={afficheNom}
            ...
            {!replie && (
              <div style={{ minWidth: 0, flex: 1 }}>
                <p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: "#101a2e", overflow: "hidden",
                ...
                <span style={{ display: "inline-flex", marginTop: 2, fontSize: 8.5, fontWeight: 800, color: ORANGE,
                  background: "rgba(202,99,31,0.10)", padding: "1px 6px", borderRadius: 999,
        ...
        {(session?.user || AUTH_ENFORCED) && (
          <button onClick={() => signOut({ callbackUrl: "/" })} className="nav-lien rouge"
            ...
            <Ico nom="logout" couleur="#dc2626" />
            {!replie && <span className="nav-txt" style={{ fontWeight: 600, color: "#dc2626" }}>Se déconnecter</span>}
          </button>
        )}
        ...
        <button onClick={basculer} className="nav-lien" aria-expanded={!replie}
          ...
          <span style={{ width: 22, display: "flex", justifyContent: "center", flexShrink: 0 }}>
            {replie ? <ChevronRight size={16} style={{ color: "#9aa5b4" }} /> : <ChevronLeft size={16} style={{ color: "#9aa5b4" }} />}
          </span>
          {!replie && <span className="nav-txt" style={{ fontWeight: 600, color: "#9aa5b4" }}>Replier</span>}
        </button>
      </div>
    </aside>
  );
