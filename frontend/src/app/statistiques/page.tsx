...
// ── Sélecteur de pays pour la Fiche Pays (overlay, jusqu'à 4 pays) ─────────────
...
export default function StatistiquesPage() {
  ...
  const [fichePicker, setFichePicker] = useState(false);
  const [ficheSel, setFicheSel] = useState<number[]>([]);
...
.drs-thumb::-moz-range-thumb{background:#004f91;border:2px solid #fff;border-radius:50%;box-shadow:0 1px 4px rgba(0,79,145,0.35);cursor:pointer;height:16px;width:16px;pointer-events:all}`}</style>
      ...
      <BarreTitre titre="Statistiques"
        droite={<BarreTitreBadge label="Fiche Pays" icon={<FileText size={13} style={{ color: "#fff" }} />} onClick={() => setFichePicker(true)} />}>
      ...
      {fichePicker && <FichePaysPicker pays={pays} senId={senId} initial={ficheSel.length ? ficheSel : selection}
        onValider={(ids) => { setFicheSel(ids); setFichePicker(false); setFicheOuverte(true); }} onClose={() => setFichePicker(false)} />}
      {ficheOuverte && <FicheComparaison paysIds={ficheSel.length ? ficheSel : selection} pays={pays} onClose={() => setFicheOuverte(false)} />}
