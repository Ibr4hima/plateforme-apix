...
// investissements (bandeau dégradé, sommaire A-Z, filtres, recherche). Le
...
import { LEXIQUE, CAT_COULEUR, type Categorie } from "@/lib/lexique";

const BLEU = "#004f91", ORANGE = "#ca631f", ENCRE = "#101a2e";
const norm = (s: string) => s.normalize("NFD").replace(/[̀-ͯ]/g, "").toLowerCase();
const ALPHABET = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("");
const CATS = Object.keys(CAT_COULEUR) as Categorie[];

export default function LexiquePage() {
  const [q, setQ] = useState("");
  const [cat, setCat] = useState<Categorie | "Tous">("Tous");
  const contentRef = useRef<HTMLDivElement>(null);

  // Termes filtrés (catégorie + recherche), triés puis groupés par 1ʳᵉ lettre
  const { groupes, lettresPresentes } = useMemo(() => {
    const nq = norm(q.trim());
    const filtres = LEXIQUE
      .filter((t) => cat === "Tous" || t.categorie === cat)
  ...
  return (
    <div style={{ fontFamily: "var(--font-google-sans)", background: "var(--ds-fond, #F7F6F5)", minHeight: "100vh" }}>
      <style>{`
        ...
        @media (max-width: 760px) { .lex-grille { grid-template-columns: 1fr; } .lex-az { position: static !important; } }
      ...
      <div style={{ background: "linear-gradient(155deg,#002a52 0%,#003a6e 35%,#004f91 70%,#1a6ab0 100%)", color: "#fff", padding: "32px 40px 92px" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 16, minWidth: 0 }}>
              ...
              <div style={{ minWidth: 0 }}>
                <p style={{ fontSize: 11, fontWeight: 800, letterSpacing: "0.22em", textTransform: "uppercase", color: "rgba(255,255,255,0.55)", margin: "2px 0 8px" }}>
                  APIX Sénégal · Comprendre l&apos;investissement
                </p>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", marginTop: 20 }}>
            <div style={{ display: "inline-flex", gap: 3, padding: 3, background: "rgba(255,255,255,0.10)", border: "1px solid rgba(255,255,255,0.18)", borderRadius: 999, flexWrap: "wrap" }}>
              {(["Tous", ...CATS] as const).map((c) => {
                const actif = cat === c;
                return (
                  <button key={c} onClick={() => setCat(c)}
                    style={{ padding: "6px 13px", borderRadius: 999, border: "none", cursor: "pointer", fontSize: 11.5, fontWeight: 800,
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "0 40px 80px" }}>
        <div className="lex-grille" style={{ marginTop: -52 }}>
          ...
          {/* Index A-Z */}
          <aside className="ds-carte lex-az" style={{ position: "sticky", top: 20, padding: "14px 8px", alignSelf: "start" }}>
                  ...
                  {termes.map((t) => (
                    <div key={t.terme} style={{ background: "#FAFAF9", border: "1px solid #F0EEEC", borderRadius: 12, padding: "15px 18px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 7 }}>
                        ...
                        <span style={{ fontSize: 10, fontWeight: 700, color: CAT_COULEUR[t.categorie], background: `${CAT_COULEUR[t.categorie]}14`, border: `1px solid ${CAT_COULEUR[t.categorie]}2e`, padding: "2px 9px", borderRadius: 999, whiteSpace: "nowrap" }}>
                          {t.categorie}
                        </span>
                      </div>
