...
function ModalBdefTable({ open, onClose, blocs, annees }: {
  ...
  const dialTable = useDialogue(open, "Tableau de données BDEF");
  if (!open) return null;
...
function MiniModalBdefKpi({ ind, annees, libelle, onClose }: {
  ...
  const dialKpi = useDialogue(!!ind, "Fiche de l'indicateur");
  if (!ind) return null;
            ...
            <>
              ...
              {(()=>{
                ...
                const renderItem = (n:BdefNode)=>{
                  ...
                  return (
                    <div key={n.id} {...carteCliquable(()=>{ if(!disabled) toggleComp(n.id); })} aria-disabled={disabled||undefined}
        ...
        {sousVue==="comparative" ? (
          ...
          <div>
            ...
            {compSelec.length===0 ? (
              ...
              <SkeletonChartGrid n={8} cols={1} height={228}/>
            ) : (
              <>
                {/* Comparaison d'entites : un graphe par ligne aussi, mais sans
                    chiffres a droite — plusieurs entites sont tracees, il n'y a
                    pas un niveau unique a afficher. */}
                  ...
                  {BDEF_GRAPHES_DEFAUT.map(code=>{
                    ...
                    return (
                      <GrapheCard key={code} titre={(compData[compSelec[0]]||[]).find(i=>i.code===code)?.libelle||code} series={series} grapheId={code} hideLegend hideSousTitre
                        ...
                        <GrapheMultiPays series={series} height={220} type="line" fmt={fmt} showDots={false} lineWidth={1.4}/>
                      </GrapheCard>
                    );
                  }).filter(Boolean)}
                </div>
