  ...
  return (
    <main style={{
      ...
      <div style={{ position: "relative", zIndex: 1, flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "20px" }}>
        ...
        <div className="login-brand" style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 22 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 13 }}>
            ...
            <p style={{ margin: 0, fontSize: "var(--t-105)", fontWeight: 700, color: "rgba(255,255,255,0.72)", letterSpacing: "0.16em", textTransform: "uppercase", textAlign: "center" }}>
        ...
        <div style={{ position: "relative", width: "100%", maxWidth: 408, display: "flex", justifyContent: "center" }}>
          ...
          <div className="login-card" style={{ width: "100%", maxWidth: 408, background: "var(--carte)", borderRadius: 20, overflow: "hidden", border: "1px solid rgb(var(--bleu-rgb) / 0.10)", boxShadow: "0 30px 70px rgb(var(--ombre-rgb) / 0.35), 0 4px 14px rgb(var(--ombre-rgb) / 0.12)", position: "relative" }}>
            ...
            <div style={{ padding: "30px 34px 28px", textAlign: "center" }}>
              <span style={{ width: 46, height: 46, borderRadius: "50%", background: "rgb(var(--vert-rgb) / 0.10)", display: "inline-flex", alignItems: "center", justifyContent: "center", marginBottom: 14 }}>
                <ShieldCheck size={22} style={{ color: "var(--vert)" }} />
              </span>
              <h1 style={{ fontWeight: 800, fontSize: "var(--t-r120)", color: "var(--encre)", letterSpacing: "-0.02em", margin: 0 }}>Compte créé</h1>
              <p style={{ color: "var(--texte)", fontSize: "var(--t-135)", marginTop: 10, lineHeight: 1.65 }}>
                Votre compte <span style={{ fontWeight: 700, color: "var(--bleu)" }}>{email}</span>{" "}
              ...
              <Link href="/login" className="login-cta"
                style={{ display: "inline-flex", alignItems: "center", gap: 8, justifyContent: "center", marginTop: 20, height: 44, padding: "0 26px", borderRadius: 12, background: "var(--bleu-action)", color: "var(--sur-bleu)", fontWeight: 700, fontSize: "var(--t-14)", textDecoration: "none", boxShadow: "0 4px 18px rgb(var(--ombre-rgb) / 0.35)" }}>
                Retour à la connexion <ArrowRight size={15} className="cta-arrow" />
              </Link>
            </div>
            ) : (
            <div style={{ padding: "26px 34px 24px" }}>
              <h1 style={{ fontWeight: 800, fontSize: "var(--t-r160)", color: "var(--encre)", letterSpacing: "-0.02em", margin: 0 }}>Créer un compte</h1>
              <p style={{ color: "var(--gris)", fontSize: "var(--t-13)", marginTop: 6, marginBottom: 0, lineHeight: 1.55 }}>
                Réservé aux agents disposant d&apos;une adresse <span style={{ color: "var(--texte)", fontWeight: 600 }}>@apix.sn</span>
              </p>

              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 13, marginTop: 20 }}>
                <div>
                  <label style={{ display: "block", fontSize: "var(--t-105)", fontWeight: 800, letterSpacing: "0.1em", color: "var(--bleu)", textTransform: "uppercase", marginBottom: 6 }}>Adresse email</label>
                  <div className="login-field">
                    ...
                    <input
                      ...
                      style={{ width: "100%", height: 43, padding: "0 14px 0 42px", border: "1.5px solid var(--bordure-forte)", borderRadius: 12, fontSize: "var(--t-14)", color: "var(--encre)", background: "var(--carte-douce)", fontFamily: "var(--font-google-sans)", boxSizing: "border-box" }}
                    />
                  </div>
                </div>

                <div>
                  <label style={{ display: "block", fontSize: "var(--t-105)", fontWeight: 800, letterSpacing: "0.1em", color: "var(--bleu)", textTransform: "uppercase", marginBottom: 6 }}>Mot de passe</label>
                  <div className="login-field">
                    ...
                    <input
                      ...
                      style={{ width: "100%", height: 43, padding: "0 44px 0 42px", border: "1.5px solid var(--bordure-forte)", borderRadius: 12, fontSize: "var(--t-14)", color: "var(--encre)", background: "var(--carte-douce)", fontFamily: "var(--font-google-sans)", boxSizing: "border-box" }}
                    />
                ...
                {password && (
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3px 12px", margin: "-4px 2px 0" }}>
                    {regles.map(r => (
                      <span key={r.l} style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: "var(--t-105)", fontWeight: 600, color: r.ok ? "var(--vert)" : "var(--gris)", transition: "color 0.15s" }}>
                        <span style={{ width: 12, height: 12, borderRadius: "50%", background: r.ok ? "rgb(var(--vert-rgb) / 0.12)" : "var(--fond-creux)", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: "var(--t-85)", lineHeight: 1, flexShrink: 0 }}>{r.ok ? "✓" : ""}</span>
                        {r.l}
                      </span>
                    ))}
                  </div>
                ...
                <div>
                  <label style={{ display: "block", fontSize: "var(--t-105)", fontWeight: 800, letterSpacing: "0.1em", color: "var(--bleu)", textTransform: "uppercase", marginBottom: 6 }}>Confirmer le mot de passe</label>
                  <div className="login-field">
                ...
                {error && (
                  <div className="login-error" style={{ background: "rgb(var(--danger-rgb) / 0.06)", border: "1px solid rgb(var(--danger-rgb) / 0.20)", color: "var(--danger)", fontSize: "var(--t-125)", fontWeight: 500, padding: "9px 13px", borderRadius: 10, textAlign: "center" }}>
                    {error}
                  </div>
                )}

                <button type="submit" disabled={loading} className="login-cta"
                  style={{ width: "100%", height: 46, borderRadius: 12, border: "none", cursor: "pointer", background: "var(--bleu-action)", color: "var(--sur-bleu)", fontWeight: 700, fontSize: "var(--t-14)", letterSpacing: "0.01em", boxShadow: "0 4px 18px rgb(var(--ombre-rgb) / 0.35)", fontFamily: "var(--font-google-sans)", marginTop: 2 }}>
                  {loading ? (
            ...
            {/* Pied de carte */}
            <div style={{ padding: "12px 34px", borderTop: "1px solid var(--bordure)", background: "var(--carte-douce)", textAlign: "center", fontSize: "var(--t-13)", color: "var(--gris-fort)" }}>
              Déjà un compte ?{" "}
              <Link href="/login" className="login-link" style={{ color: "var(--bleu)", fontWeight: 700, textDecoration: "none" }}>
                Se connecter
              </Link>
            </div>
          </div>
        ...
        <div className="login-after" style={{ display: "flex", alignItems: "center", gap: 7, marginTop: 18, maxWidth: 408 }}>
          ...
          <span style={{ fontSize: "var(--t-115)", color: "var(--gris)", lineHeight: 1.5, textAlign: "center" }}>
      ...
      <footer style={{ position: "relative", zIndex: 1, padding: "16px 40px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap", borderTop: "1px solid var(--bordure)", background: "var(--carte)" }}>
        <span style={{ fontSize: "var(--t-115)", color: "var(--gris)" }}>© {new Date().getFullYear()} APIX S.A — DIPE. Tous droits réservés.</span>
        <span style={{ fontSize: "var(--t-115)", color: "var(--gris)" }}>Plateforme à usage institutionnel</span>
      </footer>
    </main>
  )
}
